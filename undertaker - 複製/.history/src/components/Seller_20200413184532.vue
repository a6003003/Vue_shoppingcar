...
export default {
  ...
  printTrade(){
      var _this = this;
      console.log("HHHHH"+this.sellergoodlist.allPayLogisticsID);
      

      this.response_url = "";
      postRequest("/goods/printTrade/", {
       
        allPayLogisticsID:_this.sellergoodlist.allPayLogisticsID,
        checkMacValue:_this.sellergoodlist.checkMacValue,
        merchantID:_this.sellergoodlist.merchantID,//超商類型
        }).then(resp => {
   ...
   var _this = this;
    getRequest("/goods/getsellerlist/" + this.$store.getters.name).then(
      resp => {
        ...
        console.log("ABC"+_this.sellergoodlist);
        console.log("ABCD"+_this.sellergoodlist.goodlistEntitys.allPayLogisticsID);
      }
    );
  }
};

...
export default {
  ...
  methods: {
    ...
    sendMessage(msg) {//傳遞訊息事件
      var jsonObject = {
        code: this.nowchatname,//receiver
        data: {
          name: this.$store.getters.name,//sender
          message: this.inputValue//訊息
        },
        message: this.inputValue,
        roomid: this.nowchatroomId,//房號
      ...
      this.inputValue = "";
      var _this = this;
      this.socket.emit(msg, jsonObject, function(result) {// 向伺服端送消息
      });
  ...
  mounted() {
    var opts = {
      query: "clientName=" + this.$store.getters.name//誰連線
    };
    this.socket = io.connect("wss://61.71.47.141:8091", opts);//連接伺服器端  wss websocket 443port
   
    this.socket.on("receiveMessage", function(data, ackServerCallback) {//監聽伺服器訊息
